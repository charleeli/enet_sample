...
timer_create2()
{
    ...
    timer = (timer_s *)malloc(sizeof(*timer));
    if (timer) {
...
void
timer_expire(timer_s *timer, timer_execute_ptr execute, void *userp)
{
    unsigned long id;
...
void
timer_add(timer_s *timer, unsigned long id, unsigned long elapse)
{
    timer_node_t *timer_node;
...
void 
timer_delete2(timer_s *timer, unsigned long id)
{
    rbtree_node_t *node;
...
void
timer_delete_all(timer_s *timer)
{
